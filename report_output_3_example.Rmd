...
additional steps. __Write a function, `read_data`, that reads in the example
intensity data.csv as a *dataframe*. Return the first row of the dataframe
below__:
...
Now that we have performed PCA, we are interested in the amount of variance
explained by each principal component. __Write a function, `calculate_variance`,
that calculates the variance explained by each principal component from the
`pca_results` object and return a *vector* of those values below.__
...
pc_variances <- calculate_variance(pca_results)
pc_variances
```

## 3. Make a tibble with variance explained and cumulative variance explained for the PCs
__Using the vector of variances you just generated, write a function,
`make_variance_tibble`, that uses this vector along with the
`pca_results` object to construct a *tibble* with three columns containing the
name of each PC, the variance explained by each PC, and the cumulative sum of
variance explained. Return the first five rows below.__

```{r code block to construct a tibble with PCs, variance explained, and cumulative variance explained}
variance_tibble <- make_variance_tibble(pc_variances, pca_results)
...
`{r code}` chunk for these parts (4, 5, 8), you will notice the fig.caption flag
...
scatterplot (connected with a line) that displays the cumulative variance across
all the PCs. Provide appropriate labels for each plot.__
...
__Write a function, `make_biplot` that generates a *plot* of the first two principal
components labeled by their SixSubTypesClassification found in the
`proj_metadata.csv`.__
...
variance, the scale of the axes will not be the same.
...
significant probes with a p-value of less than .01, and returns a *list* of the
...
value for the p-value threshold.
...
you construct this function with a p-value argument so that theoretically, in
the future, you could easily re-run it with a different value. For our purposes,
we will manually set this argument as .01.
...
significantly differentially expressed and plot them in a heatmap. __Write a
function, `return_de_intensity`, that uses the list of significant probes you
generated in part 6 `sig_ids` and the example intensity data from part 1
`intensity` to return a *matrix* of intensity values for just the significant
probes.__
...
__Write a function, `heatmap_plot`, that usesthe `de_intensity` results as and 
returns a heatmap *plot*.__
